function ObjObject(								//声明绘制用物体对象所属类
    ...
    this.vertexTexCoor=vertexTexCoorIn;             //接收顶点纹理坐标数据缓冲
    this.vertexTexCoorBuffer=gl.createBuffer();     //创建顶点纹理坐标数据缓存
    gl.bindBuffer(gl.ARRAY_BUFFER,this.vertexTexCoorBuffer);//绑定纹理坐标数据缓存
    ...
    this.drawSelf=function(ms,texture)//绘制物体的方法
    {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER,this.vertexTexCoorBuffer);//绑定顶点纹理坐标数据缓冲
